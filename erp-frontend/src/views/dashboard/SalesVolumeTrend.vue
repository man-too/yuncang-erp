  ...
  <el-card shadow="never" class="purchase-card">
    <template #header>
      <div class="card-header">
        <span class="card-title">采购金额</span>
    ...
    <div v-if="!store.purchaseAmountData" class="chart-empty">暂无数据</div>
    <v-chart
      v-else
      :option="chartOption"
      autoresize
...
watch([selectedProduct, selectedDays], () => {
  store.fetchPurchaseAmount({
...
const chartOption = computed(() => {
  const d = store.purchaseAmountData
  if (!d) return {}
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        const val = typeof p.value === 'number' && p.value >= 10000
          ...
          : `¥${p.value}`
        return `${p.axisValue}<br/>${p.marker} 采购金额: ${val}`
      },
    },
...
<style scoped>
.purchase-card { border-radius: 8px; }
.card-header {
  display: flex;
